...
export const sourceInputSchema = z.object({
  ...
  trustLevel: z.enum(["PRIMARY", "HIGH", "MEDIUM", "LOW"]),
  notes: z.string().trim().min(3).max(10000),
});

export const claimInputSchema = z.object({
  claimText: z.string().trim().min(5).max(2000),
  sourceId: z.string().uuid().nullable().optional(),
  supportStatus: z.enum(["SUPPORTED", "PARTIAL", "UNSUPPORTED", "STALE"]),
  riskLevel: z.enum(["LOW", "MEDIUM", "HIGH"]),
  evidence: z.string().trim().min(3).max(5000),
